...
sub slice {
    ...
    for my $region_id (0 .. $#{$self->meshes}) {
        ...
        # build XS mesh
        my $mesh_xs = Slic3r::TriangleMesh::XS->new;
        $mesh_xs->ReadFromPerl($mesh->vertices, $mesh->facets);
        $mesh_xs->Repair;
        
        # slice
        my @z = grep defined $_, map $_->slice_z, @{ $self->layers };
        my $loops = $mesh_xs->Slice(\@z);
        ...
        foreach my $layer (@{ $self->layers }) {
            ...
            $layerm->make_surfaces(shift @$loops);
        }
    }
